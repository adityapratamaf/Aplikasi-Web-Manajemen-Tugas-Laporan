			...
			<table class="table tabe-hover table-condensed" id="list">
				...
				<tbody>
					...
					$qry = $conn->query("SELECT t.*,p.name as pname,p.start_date,p.status as pstatus, p.end_date,p.id as pid 
							FROM task_list t inner join project_list p on p.id = t.project_id $where order by p.name asc");
						...
						<tr>
							...
							<td>
								...
								<div class="dropdown-menu" style="">
									<a class="dropdown-item new_productivity" data-pid='<?php echo $row['pid'] ?>' data-tid='<?php echo $row['id'] ?>' data-task='<?php echo ucwords($row['task']) ?>' href="javascript:void(0)">Tambah Laporan</a>
								</div>
							</td>
						</tr>
					<?php endwhile; ?>
...
<script>
	$(document).ready(function() {
		...
		$('.new_productivity').click(function() {
			uni_modal("<i class='fa fa-edit'></i> Laporan Tugas : " + $(this).attr('data-task'), "manage_progress.php?pid=" + $(this).attr('data-pid') + "&tid=" + $(this).attr('data-tid'), 'large')
		})
	})
